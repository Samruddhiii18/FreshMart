...
function TrackOrder({params}:{params:{orderId:string}}) {
  ...
  useEffect(():any => {
    ...
    socket.on("update-deliveryBoy-location",(data) => {
      // if(data.userId !== order?.assignedDeliveryBoy?._id?.toString()) return
      setDeliveryBoyLocation({
      ...
      <div className='max-w-2xl mx-auto pb-24'>
        ...
        <div className='flex gap-2 flex-wrap mb-3'>
            ...
            <div className='flex-1 overflow-y-auto p-2 space-y-3' ref={chatBoxRef}>
                <AnimatePresence>
                    {messages?.map((msg,index) => (
                        <motion.div
                        key={msg._id?.toString() || `message-${index}`}
            ...
            <div className='flex gap-2 mt-3 border-t pt-3'>
              <input type="text" placeholder='Type a Message....' className='flex-1 bg-gray-100 px-4 py-2 
              rounded-xl outline-none focus:ring-2 focus:ring-green-500' value={newMessage} onChange={(e) => setNewMessage(e.target.value)}/>
