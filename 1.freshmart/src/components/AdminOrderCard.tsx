    ...
    className='bg-white shadow-md hover:shadow-lg border border-gray-100 rounded-2xl p-6 transition-all'>
        ...
        <div className='flex flex-col md:flex-row md:items-start md:justify-between gap-4'>
            ...
            <div className='space-y-1'>
                ...
                <p className='text-lg font-bold flex items-center gap-2 text-green-700'><Package size={20}/> Order #{order._id?.toString().slice(-6)}</p>
                ...
                <div className='mt-3 space-y-1 text-gray-700 text-sm'>
                    <p className='flex items-center gap-2 font-semibold'><User size={16} className='text-green-600'/><span>{order?.address.fullName}</span></p>
                    <p className='flex items-center gap-2 font-semibold'><Phone size={16} className='text-green-600'/><span>{order?.address.mobile}</span></p>
                    <p className='flex items-center gap-2 font-semibold'><MapPin size={16} className='text-green-600'/><span>{order?.address.fullAddress}</span></p>
                </div>

                <p className='mt-3 flex items-center gap-2 text-sm text-gray-700'><CreditCard size={16} className='text-green-600'/><span>{order.paymentMethod==="cod"?"Cash On Delivery":"Online Payment"}</span></p>
                ...
                {order.assignedDeliveryBoy && <div className='mt-4 bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-center justify-between'>
                    <div className='flex items-center gap-3 text-sm text-gray-700'>
                        ...
                        <div className='font-semibold text-gray-800'>
                            <p className='text-xs text-gray-600'>Assigned To: <span>{order.assignedDeliveryBoy.name}</span></p>
        ...
        <div className='border-t border-gray-200 mt-3 pt-3'>
            <button className='w-full flex justify-between items-center text-sm font-medium text-gray-700 hover:text-green-700 transition' onClick={() =>setExpanded(prev=>!prev)}>
        ...
        <div className='border-t pt-3 mt-3 flex justify-between items-center text-center text-sm font-semibold text-gray-800'>
            <div className='flex items-center gap-2 text-gray-700 text-sm'>
                <Truck size={16} className='text-green-600'/>
                <span>Delivery: <span className='text-green-700 font-semibold'>{status}</span></span>
            </div>
            <div>
                Total: <span className='font-bold text-green-700'>₹ {order.totalAmount}</span>
            </div>
        </div>
